    ...
    <header className="home-entrance-header relative z-[var(--z-header)] border-b border-[var(--color-border)] bg-[var(--color-soft-cream)]">
      <Container className="grid min-h-20 grid-cols-[1fr_auto_1fr] items-center lg:min-h-24 lg:grid-cols-[auto_1fr_auto] lg:gap-12">
        ...
        <nav aria-label="Account and shopping" className="flex items-center justify-self-end">
          ...
          <button type="button" aria-label="Open cart, 0 items" className="relative flex size-11 items-center justify-center transition-colors hover:text-[var(--color-crimson)]">
            <BagIcon />
            <span aria-hidden="true" className="absolute right-0.5 top-0.5 flex size-4 items-center justify-center rounded-full bg-[var(--color-crimson)] text-[0.55rem] font-bold text-white">0</span>
          </button>
        </nav>
      </Container>
    </header>
